...
const schema = z.object({
    ...
    numeroCilindro: z.string().max(100, "O campo numeroCilindro deve ter no máximo 100 caracteres").regex(/^\d{1,4}$/, "A penas numero"),
    ...
    numeroMatricula: z.string().max(50, "O campo numeroMatricula deve ter no máximo 50 caracteres").regex(/^[A-Z]{2}-\d{2}-\d{2}-[A-Z]{2}$/, "O formato deve ser LL-##-##-LL"),
    marca: z.string(),  // Presumindo que este campo seja opcional
    codPessoa: z.number().int().nonnegative("O campo proprietario deve ser um número inteiro não negativo"),
    ...
    return (
        <FormProvider {...form}>
            ...
            <form onSubmit={form.handleSubmit(handleSubmitAutomobilista)}>
                {step == 1 && <SecondFormViatura setNextStep={setNextStep} setPreviusStep={setPreviusStep} />}
                {step == 2 && <FirstFormViatura setNextStep={setNextStep} setPreviusStep={setPreviusStep} />}
