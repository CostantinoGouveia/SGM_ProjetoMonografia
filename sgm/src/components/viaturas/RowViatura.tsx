...
export default function RowViatura({ viatura }: { viatura: Viatura }) {
    ...
    return (
        <TableRow key={viatura.numeroMatricula}>
            ...
            {isSus && dataPessoa.usuario[0].tipoUsuario == "Admin" && (<TableCell><AlertDeleteViatura id={viatura.codViatura.toString()} handleClick={handleDeleteViatura}><Button variant={"outline"} className=" hover:bg-muted"><Trash2 className="w-5 h-5 text-red-700" /></Button></AlertDeleteViatura></TableCell>)}
        </TableRow>
    )
}
