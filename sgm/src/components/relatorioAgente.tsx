...
import { GET_AUTOMOBILISTAS, GET_FUNCIONARIOS, GET_MULTAS, GET_MUNICIPIOS, GET_PROVINCIAS } from "@/routes";
import { useQuery } from "@tanstack/react-query";
import { isAfter, isBefore, isWithinInterval } from "date-fns";
import React, { useState, useEffect } from "react";

const RelatorioAgente = () => {
  ...
  const filtrarRelatorio = () => {
    ...
    return relatorio.filter((item: any) => {
      ...
      if (filtros.tipoUsuario && item.pessoa?.usuario?.[0]?.tipoUsuario.toLowerCase() !== filtros.tipoUsuario.toLowerCase()) return false;
      return true;
    });
  };
  ...
  return (
    <div className="container">
      ...
      <footer>
        <p> © {new Date().getFullYear().toLocaleString()} Lista de Agentes - Sistema de Gestão de Multas</p>
      </footer>
...
export default RelatorioAgente;
